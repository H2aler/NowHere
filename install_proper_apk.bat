@echo off
echo ========================================
echo NowHere AR MMORPG 완전한 APK 설치
echo ========================================

REM APK 파일 확인
if not exist "ProperAPK\NowHere_AR_MMORPG_Proper_v0.1.0.apk" (
    echo ❌ 완전한 APK 파일을 찾을 수 없습니다!
    echo 먼저 build_proper_apk.bat을 실행하여 APK를 빌드해주세요.
    pause
    exit /b 1
)

echo ✅ 완전한 APK 파일 확인됨: ProperAPK\NowHere_AR_MMORPG_Proper_v0.1.0.apk
...
powershell -command "try { Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::OpenRead('ProperAPK\NowHere_AR_MMORPG_Proper_v0.1.0.apk') | Out-Null; Write-Host '✅ APK 파일 형식이 올바릅니다.' } catch { Write-Host '❌ APK 파일 형식에 문제가 있습니다.' }"
...
if "%choice%"=="4" goto copy_to_downloads
goto invalid_choice
...
echo.
echo 📱 APK 설치 중...
adb install -r "ProperAPK\NowHere_AR_MMORPG_Proper_v0.1.0.apk"
...
echo 📋 수동 설치 단계:
echo.
echo 1. 📁 APK 파일 위치: ProperAPK\NowHere_AR_MMORPG_Proper_v0.1.0.apk
echo.
...
echo    - "Test app built successfully!" 메시지 확인
echo    - "This is a complete APK built with Android SDK." 메시지 확인
echo    - 버전 정보 확인
echo.
goto end
...
echo.
echo ========================================
echo APK 파일을 다운로드 폴더로 복사
echo ========================================

REM 다운로드 폴더 확인
if not exist "%USERPROFILE%\Downloads" (
    echo ❌ 다운로드 폴더를 찾을 수 없습니다!
    goto end
)
...
if %ERRORLEVEL% EQU 0 (
    echo.
    echo ✅ APK 파일이 다운로드 폴더로 복사되었습니다!
    echo.
    echo 📁 복사된 위치: %USERPROFILE%\Downloads\NowHere_AR_MMORPG_Proper_v0.1.0.apk
    echo.
    echo 📱 이제 이 파일을 안드로이드 기기로 전송하여 설치하세요.
    echo.
) else (
    echo.
    echo ❌ APK 파일 복사 실패!
...
echo 📦 APK 파일 구조:
echo.
echo ProperAPK\NowHere_AR_MMORPG_Proper_v0.1.0.apk
echo ├── AndroidManifest.xml (앱 매니페스트)
echo ├── resources.apk (AAPT로 컴파일된 리소스)
echo ├── META-INF\ (메타데이터)
echo └── res\ (리소스 파일들)
